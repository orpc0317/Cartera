...
export function BancosClient({
  ...
  return (
    <div className="flex flex-col gap-6 p-6 md:p-8">
                  ...
                  >
                    ...
                    {visibleCols.map((col) => {
                      switch (col.key) {
                        ...
                        case 'proyecto': return <TableCell key="proyecto" className="text-muted-foreground">{proyectoMap.get(`${banco.empresa}-${banco.proyecto}`) ?? `#${banco.proyecto}`}</TableCell>
      ...
      <p className="text-xs text-muted-foreground">{filtered.length} banco{filtered.length !== 1 ? 's' : ''}</p>
