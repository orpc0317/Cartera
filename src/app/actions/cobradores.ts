...
import type { Cobrador, CobradorForm } from '@/lib/types/proyectos'
...
export async function getCobradores(empresa?: number, proyecto?: number): Promise<Cobrador[]> {
...
export async function createCobrador(form: CobradorForm): Promise<{ error?: string }> {
  const cuenta = await getCuentaActiva()
  if (!cuenta) return { error: 'Sesión no válida.' }
  const permCheck = await requirePermiso(PERMISOS.COB_CAT, 'agregar')
  ...
  const { data: existente } = await admin
    ...
    .maybeSingle()
  if (existente) return { error: 'Ya existe un cobrador con ese nombre en este proyecto.' }
  ...
  await writeAudit(admin, {
    tabla: 't_cobrador', operacion: 'INSERT', cuenta,
    registroId: { empresa: form.empresa, proyecto: form.proyecto, codigo: (data as Cobrador).codigo },
    datoAntes: null, datoDespues: data as Record<string, unknown>,
    ...auditUser,
  })

  revalidatePath('/dashboard/promesas/cobradores')
...
export async function updateCobrador(
  ...
  proyecto: number,
  codigo: number,
  form: Partial<CobradorForm>,
  lastModified?: string,
): Promise<{ error?: string }> {
  const cuenta = await getCuentaActiva()
  if (!cuenta) return { error: 'Sesión no válida.' }
  const permCheck = await requirePermiso(PERMISOS.COB_CAT, 'modificar')
  if (permCheck) return permCheck
  const [auditUser, admin] = [await getAuditUser(), createAdminClient()]
  ...
  await writeAudit(admin, {
    tabla: 't_cobrador', operacion: 'UPDATE', cuenta,
...
export async function deleteCobrador(
  empresa: number,
  proyecto: number,
  codigo: number,
): Promise<{ error?: string }> {
  const cuenta = await getCuentaActiva()
  if (!cuenta) return { error: 'Sesión no válida.' }
  const permCheck = await requirePermiso(PERMISOS.COB_CAT, 'eliminar')
  if (permCheck) return permCheck
  const [auditUser, admin] = [await getAuditUser(), createAdminClient()]
  ...
  await writeAudit(admin, {
    tabla: 't_cobrador', operacion: 'DELETE', cuenta,
    registroId: { empresa, proyecto, codigo },
    datoAntes: oldRow as Record<string, unknown> | null,
    datoDespues: null,
    ...auditUser,
