...
> - **Si existe** ? **DETENER. Preguntar al desarrollador** si desea sobrescribir. No continuar hasta recibir confirmaci�n expl�cita de que s�.
...
> `getEmpresas`, `getProyectos` y `getSeriesFactura` aplican `.eq('cuenta', cuenta)` internamente � el campo `cuenta` no aparece en ning�n Select ni prop visible.
...
| recibo_automatico | Automatico      | true           | S�/No                                                |
| correlativo       | Correlativo     | false          | valor directo                                         |
| predeterminado    | Predeterminado  | true           | S�/No                                                |
...
| Modo   | Título                      |
|--------|-----------------------------|
| nuevo  | Nueva Serie de Recibos      |
| editar | Editar Serie de Recibos     |
| ver    | {serie}                     |
...
| serie_factura     | Serie Factura  | half  | ViewField              | Select FK [�F]                               | null            | ver REGLA #3; nullable; primer item = vac�o |
...
3. `serie_factura` es un `<Select>` (FK de `t_serie_factura`), **no** un input de texto libre.
   ...
   - No aplica normalizaci�n `f()` ni `toUpperCase()` para este campo; agregarlo a `SKIP_KEYS` para que `f()` no lo toque.
